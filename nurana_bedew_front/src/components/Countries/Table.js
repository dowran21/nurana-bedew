...
import { get_producers} from '../../http/adminApi.js';
...
function Table(search){
    ...
    return(
        ...
        <div className = "w-full">
            <div>
                <ModalContainer Button={(setClose) => <AddIcon onClick={() => setClose()} className="w-20 h-10 p-1 cursor-pointer text-indigo-600 hover:text-indigo-900"/>} title="Öndürijini goşmak" size="min">
                    <Form setProducers = {setProducers} producers = {producers}></Form>
                </ModalContainer>
            </div>
        <div>
            ...
            {producers && producers.map(item => 
                ...
                <td className="py-2 px-2">
                        <div className="flex flex-row justify-between items-center">
                            <ModalContainer Button={(setClose) => <EditIcon onClick={() => setClose()} className="w-5 h-5 mx-3 cursor-pointer text-indigo-600 hover:text-indigo-900"/>} title="Öndürijini Üýtgetmek Formasy" size="min">
                                <Form values = {{...item}} setLoad = {setLoad} producers = {producers} setProducers = {setProducers}></Form>
                            </ModalContainer>
                        </div>
                </td>
            </tr>
